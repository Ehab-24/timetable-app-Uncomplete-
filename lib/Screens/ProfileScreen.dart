  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        const BackgroundText(title: 'Profile'),
        ...
        BackgroundText(title: 'Today', subtitle: DateFormat.MMMd().format(DateTime.now()),),

        Spaces.vertical20,

        _TodaysTasks(animate: ticker > 3,),

        Spaces.vertical60,

        _TasksPercentages(animate: ticker > 5,),

        Spaces.vertical40,

        _CurrentStats(animate: ticker > 3,),

        const HorzDivider(),

        const BackgroundText(title: 'Settings'),

        _Settings(animate: ticker > 3,),
              ...
              Row(
                ...
                children: [
                  ...
                  _ToggleButton(
                    ...
                    onPressed: (){
                    setState(() => blurHomeScreenImage = !blurHomeScreenImage);
                  },),
                  _ToggleButton(
                    icon: Icons.gradient,
                    label: 'Gradient',
                    ...
                    onPressed: (){
                    setState(() => gradientHomeScreenImage = !gradientHomeScreenImage);
                  },),
                ],
              )
          ],
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        GestureDetector(  
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              color: Colors.white,
              border: Border.all(color: comparator? Colors.pink: Colors.blueGrey.shade800, width: 2)
            ),
            child: Icon(
              icon, 
              size: comparator? 28: 22, 
              color: comparator? Colors.pink: const Color.fromRGBO(55, 71, 79, 0.5),
            ),
          ),
        ),
        ...
        Text(
          label,
          style: TextStyles.h8(comparator? Colors.pink: Colors.blueGrey.shade800),
        )
      ],
    );
  }
        ...
        child: PhysicalModel(
          ...
          child: Container(
            ...
            child: Row(
              ...
              children: const [
                _NumberWithLabel(label: 'No. of Tables', number: '3',),
                VertDividerMini(),
                _NumberWithLabel(label: 'Work Load Hours', number: '47',),
                VertDividerMini(),
                _NumberWithLabel(label: 'Reminders', number: '6',),
              ],
            ),
          ),
        ),
...
class _TasksPercentages extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceAround,
      children: [
        _ProgressIndicator(content: '92.78', label: 'COMPLETED', animate: animate,),
        _ProgressIndicator(content: '7.22', label: 'LEFT', animate: animate,)
      ],
    );
  }
}
    ...
    return AnimatedOpacity(
      ...
      child: PhysicalModel(
        ...
        child: AnimatedContainer(
          ...
          child: Column(
            children: const [
              _SwicthWithHeader(title: 'Dark Mode',),
              _SwicthWithHeader(title: 'Auto-dispose Reminders',),
              HorzDividerMini(),
              _SwicthWithHeader(title: 'Notifications',),
            ],
          ),
        ),
      ),
...
class _TodaysTasksState extends State<_TodaysTasks> {
          ...
          child: AnimatedContainer(
            ...
            child: Column(
              children: [
                ...
                _TaskTitles(header: 'Done', tasks: _tasksCompleted.isEmpty? ['None']: _tasksCompleted),
                
                const HorzDividerMini(),
                
                _TaskTitles(header: 'Left', tasks: _tasksLeft.isEmpty? ['None']: _tasksLeft),
              ],
            )
          ),
        ),
  ...
  void fillTaskLists() {
    final List<TimeSlot> todaysTimeSlots = tableReader.tables[Prefs.homeTable].timeSlots[DateTime.now().weekday - 1];
...
class _TaskTitles extends StatelessWidget {
  ...
  }) : _tasksCompleted = tasks, super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      children: [
        ...
        Text('$header:   ', style: TextStyles.b1(colorWatch.foreground),),

        Wrap(
          children: _tasksCompleted.map((title) =>
            Text(
              '$title   ', 
              style: TextStyles.h4(colorWatch.foreground.withOpacity(0.7)),
            )).toList()
        )
      ],
    );
    ...
    return Row(
      children: [
        ...
        Switch(
          ...
          ? Prefs.isDarkMode 
          : widget.title[0] == 'B'
          ? applyHomeScreenImage : autoDisposeReminders,

          onChanged: (value) =>
            setState(() => widget.title[0] == 'N'
            ? allowNotifications = value
            : widget.title[0] == 'D'
            ? value? colorReader.toDark(): colorReader.toLight()
            : widget.title[0] == 'B'
            ? applyHomeScreenImage = value : autoDisposeReminders = value
          )
        )
      ],
    );
